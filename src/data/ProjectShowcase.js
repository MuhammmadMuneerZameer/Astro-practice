// src/data/ProjectShowcase.js
const projects = [
  // {
  //   title: "Rents A Cars",
  //   description: "A modern car rental platform with real-time booking, fleet management, and seamless user experience. Built with cutting-edge web technologies for optimal performance.",
  //   image: "/images/projects/rents-a-cars.png", // Add your screenshot here
  //   technologies: ["React", "Next.js", "Tailwind CSS", "Firebase"],
  //   link: "https://rents-a-cars.vercel.app/",
  //   slug: "rents-a-cars"
  // },
  // {
  //   title: "Alpha Tool",
  //   description: "Professional business automation tool designed to streamline workflows and boost productivity. Features advanced analytics and intuitive dashboard.",
  //   image: "/images/projects/alphatool.png", // Add your screenshot here
  //   technologies: ["React", "TypeScript", "Node.js", "MongoDB"],
  //   link: "https://alphatool.hydrafoxdesigns.com",
  //   slug: "alpha-tool"
  // },
  {
    title: "Disgraced Online Store",
    description: "A stylish online store offering a curated selection of fashion apparel and accessories. Features a sleek design, easy navigation, and secure checkout process.",
    image: "/images/Disgraced.png", // Add your screenshot here
    technologies: ["React", "E-commerce", "Stripe", "Tailwind CSS"],
    link: "https://disgraced.shop/",
    slug: "aastore-online"
  },
  {
    title: "Portfolio Website",
    description: "A sleek and modern portfolio showcasing creative work with smooth animations and an intuitive user interface.",
    image: "/images/desktop.png",
    technologies: ["HTML", "CSS", "JavaScript", "GSAP"],
    link: "https://nbx-capitals-investments.netlify.app/",
    slug: "portfolio-website"
  },
  {
    title: "Medical Appointment Website",
    description: "A comprehensive medical appointment booking system with user-friendly interface, real-time availability, and secure patient data management.",
    image: "/images/websiteUI.webp",
    technologies: ["Vue.js", "Firebase", "Vuetify", "PWA"],
    link: "#",
    slug: "task-management-app"
  }
];

export default projects;